...
import progressbar
from models import BeGANSiameseCritic,DenseBeGANGenerator
from utils import data_generator, plot_pair

torch.manual_seed(0)
np.random.seed(0)

generator = DenseBeGANGenerator().to("cuda:0")
...
for epoch in range(epochs):
	...
	with progressbar.ProgressBar(max_value = steps, widgets = widgets) as bar:
		for i in range(steps):
			...
			msssim_value, gauss_window = ssim_funct.forward((y+1)/2.0, (gx+1)/2.0, return_window = True)
			msssim_loss = 1.0 - msssim_value

			mse_loss = ((y - gx)**2).mean(0).mean(1).mean(1)

			recon_loss = alpha*msssim_loss + (1.0 - alpha)*mse_loss

			gen_loss = 0.5*(recon_loss[0] + recon_loss[1])
			
			beg_loss = fake_loss_d
			loss = 1000*beg_loss + 100*gen_loss
